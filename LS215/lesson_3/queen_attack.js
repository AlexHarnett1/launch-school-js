function queenAttack(str) {
  let board = str.split('\n');
  let queens = queenCoordinates(board);
  if (!Object.hasOwn(queens, 'W') || !Object.hasOwn(queens, 'B')) {
    return undefined;
  }
  return attackableCoordiantes(queens['W'], queenLocations['B']);
}
...
function queenCoordinates(board) {
  ...
  board.forEach((element, index) => {
    if (element.includes('W')) {
      queens['W'] = [index, element.indexOf('W')];
    }
    if (element.includes('B')) {
      queens['B'] = [index, element.indexOf('B')];
    }
  });
  return queens;
}
...
function attackableCoordiantes(coordinate1, coordinate2) {
  ...
  let diagDiff1 = Math.abs(coordinate1[0] - coordinate2[0]);
  let diagDiff2 = Math.abs(coordinate1[1] - coordinate2[1]);
  if (diagDiff1 === diagDiff2) return true;
